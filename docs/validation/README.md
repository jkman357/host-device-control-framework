...
- [`Repository_Validation_Checklist.md`](Repository_Validation_Checklist.md) — **Baseline / operational checklist** for repository structure, canonical LF checkout policy, non-link filesystem-entry safety, complete CODEOWNERS coverage, canonical authority-registry paths, current Changelog authority-revision snapshot, scoped release-state claims, commit-scoped legal-anchor verification, references, metadata, Protocol schema/fixtures, CI, and package integrity.
...
- [`Protocol_Validation_Checklist.md`](Protocol_Validation_Checklist.md) — **Draft for Review / conformance checklist** for Protocol definition, `node_model`, topology, identity/addressing, targeting, scope, compatibility, security, malformed input, fixtures, independent Protocol Conformance Tester applicability, independence, staged baseline gates, exact tested identities, evidence freshness, representative-target residual boundaries, cross-field consistency, Tester self-validation, and interoperability evidence.
