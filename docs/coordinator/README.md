...
- [`Coordinator_Software_Engineering_Rules.md`](Coordinator_Software_Engineering_Rules.md) — **Baseline** cross-topic normative authority for Coordinator minimum engineering rules, replaceable Presentation boundaries, lifecycle, state ownership, communication integration, security, deviation, and release governance.
- [`Coordinator_Architecture_Patterns.md`](Coordinator_Architecture_Patterns.md) — **Baseline** Coordinator layering, replaceable Presentation adapter, Node Registry and Context patterns, immutable target binding, per-Node state/resources, shared-bus scheduling, aggregate state, multi-target operations, and architecture review.
...
- [`Coordinator_UI_Engineering_Guide.md`](Coordinator_UI_Engineering_Guide.md) — **Baseline** replaceable UI-framework boundaries, Node selection, immutable operation binding, aggregate/per-Node views, partial results, stale data, responsiveness, visualization, input safety, and engineering controls.
...
`Coordinator_Software_Engineering_Rules.md` owns cross-topic Coordinator minimum rules. A Baseline topic-specific Guide owns the detailed realization topic declared in its metadata and Authority Boundary; a Draft for Review Guide remains proposed until explicitly adopted or approved for Project use. A Guide shall not weaken the Framework, Project requirements, Protocol authority, Coordinator Software Engineering Rules, or applicable language rules.
