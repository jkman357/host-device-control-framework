...
- [`Protocol_YAML_Definition_Guide.md`](Protocol_YAML_Definition_Guide.md) — **Baseline** syntax, semantics, conditional `node_model`, machine-verifiable representation, Schema Validation, Semantic Lint, Code Generation, Golden Vector, and independent Protocol Conformance Tester applicability, independence, staged baseline lifecycle, exact tested-candidate evidence binding, release reconciliation, representative-target claim boundaries, disposition consistency, and Tester self-validation.
...
- [`Protocol_Security_Profile.md`](Protocol_Security_Profile.md) — **Draft for Review / proposed normative authority** for security applicability, per-Node identity and Session binding, authorization, replay, Counters, Rekey, reconnect, credentials, Bootloader separation, and Firmware Update security.
