...
def _redact_value(value: Any, depth: int = 0) -> Any:
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            key_text = str(key)
            result[key_text] = (
                "<redacted>" if SENSITIVE_KEY_PATTERN.search(key_text)
...
def _safe_actual(value: Any) -> str:
    ...
    return text[: MAX_ACTUAL_LENGTH - 3] + "..." if len(text) > MAX_ACTUAL_LENGTH else text
...
def _schema_issues(document: Any, schema: dict[str, Any]) -> list[ValidationIssue]:
    ...
    for error in errors:
        issues.append(
            ValidationIssue(
                "PY-SCHEMA-001", _json_path(error.absolute_path), error.message,
...
def _require_mapping(
    ...
    if not isinstance(value, dict):
        issues.append(
            ValidationIssue(
                "PY-MN-002", f"{path}.{key}", "value must be a mapping",
...
def _validate_legacy_profile(document: dict[str, Any]) -> list[ValidationIssue]:
    ...
    if schema_version not in LEGACY_SCHEMA_VERSIONS:
        issues.append(
            ValidationIssue(
                "PY-LEGACY-001", "$.schema_version",
                "node_model omission is permitted only for an explicitly supported legacy schema",
                expected=str(sorted(LEGACY_SCHEMA_VERSIONS)), actual=_safe_actual(schema_version),
                correction=(
                    "Use schema_version 1.0 with the complete legacy profile or define node_model."
                ),
            )
        )
    issues.extend(
...
def validate_document(
    ...
    if not isinstance(document, dict):
        return [ValidationIssue(
            "PY-ROOT-001", "$", "Protocol YAML root must be a mapping",
            expected="mapping", actual=type(document).__name__,
        )]
    missing = sorted(ROOT_REQUIRED - set(document))
    if missing:
        return [ValidationIssue(
            "PY-ROOT-002", "$", "required top-level sections are missing",
            expected=", ".join(sorted(ROOT_REQUIRED)), actual=", ".join(missing),
    ...
    if not isinstance(node_model, dict):
        return [ValidationIssue(
            ...
            expected="mapping", actual=type(node_model).__name__,
        )]
    missing_node = sorted(NODE_REQUIRED - set(node_model))
    if missing_node:
        issues.append(ValidationIssue(
            "PY-MN-003", "$.node_model", "required Multi-Node fields are missing",
            expected=", ".join(sorted(NODE_REQUIRED)), actual=", ".join(missing_node),
        ))
    ...
    if topology not in TOPOLOGIES:
        issues.append(ValidationIssue(
            "PY-MN-004", "$.node_model.topology", "unsupported topology",
            expected=str(sorted(TOPOLOGIES)), actual=_safe_actual(topology),
        ))
    if not isinstance(maximum_nodes, int) or isinstance(maximum_nodes, bool) or maximum_nodes < 1:
        issues.append(ValidationIssue(
            "PY-MN-004", "$.node_model.maximum_nodes",
            "maximum_nodes must be an integer greater than zero",
            expected=">= 1", actual=_safe_actual(maximum_nodes),
        ))
    if not isinstance(maximum_online, int) or isinstance(maximum_online, bool) or maximum_online < 1:
        issues.append(ValidationIssue(
            "PY-MN-005", "$.node_model.maximum_online_nodes",
            "maximum_online_nodes must be an integer greater than zero",
            expected=">= 1", actual=_safe_actual(maximum_online),
        ))
    if (
        isinstance(maximum_nodes, int) and not isinstance(maximum_nodes, bool)
        and isinstance(maximum_online, int) and not isinstance(maximum_online, bool)
        and maximum_online > maximum_nodes
    ):
        issues.append(ValidationIssue(
            "PY-MN-006", "$.node_model.maximum_online_nodes",
            "maximum_online_nodes cannot exceed maximum_nodes",
            expected=f"<= {maximum_nodes}", actual=str(maximum_online),
        ))
    if topology == "single_node" and maximum_nodes != 1:
        issues.append(ValidationIssue(
            "PY-MN-007", "$.node_model.maximum_nodes",
    ...
    if identity and identity.get("node_id_required") is not True:
        issues.append(ValidationIssue(
            "PY-MN-010", "$.node_model.identity.node_id_required",
            "explicit node_model profiles require stable Node identity",
            expected="true", actual=_safe_actual(identity.get("node_id_required")),
        ))
    if identity and identity.get("uniqueness_scope") not in {"protocol_instance", "product", "global"}:
        issues.append(ValidationIssue(
            "PY-MN-011", "$.node_model.identity.uniqueness_scope",
    ...
    if method not in ADDRESS_METHODS:
        issues.append(ValidationIssue(
            "PY-MN-015", "$.node_model.addressing.method", "unsupported addressing method",
            expected=str(sorted(ADDRESS_METHODS)), actual=_safe_actual(method),
        ))
    if topology == "shared_multidrop_bus" and method != "frame_address":
        issues.append(ValidationIssue(
            "PY-MN-016", "$.node_model.addressing.method",
            "shared_multidrop_bus requires frame_address",
            expected="frame_address", actual=_safe_actual(method),
        ))
    if topology in {"shared_multidrop_bus", "routed_gateway"} and target_on_wire is not True:
        issues.append(ValidationIssue(
            "PY-MN-017", "$.node_model.addressing.target_required_on_wire",
            "shared or routed topology requires an on-wire target",
            expected="true", actual=_safe_actual(target_on_wire),
        ))
    if topology in {"single_node", "independent_links"} and method == "connection_bound" and target_on_wire is not False:
        issues.append(ValidationIssue(
            "PY-MN-018", "$.node_model.addressing.target_required_on_wire",
            "connection-bound addressing must not duplicate the target on wire",
            expected="false", actual=_safe_actual(target_on_wire),
        ))

    broadcast = addressing.get("broadcast") if addressing else None
    if not isinstance(broadcast, dict):
        issues.append(ValidationIssue(
            "PY-MN-019", "$.node_model.addressing.broadcast",
            "broadcast must be a mapping", expected="mapping",
        ...
        if not isinstance(supported, bool):
            issues.append(ValidationIssue(
                "PY-MN-020", "$.node_model.addressing.broadcast.supported",
                "broadcast supported must be boolean", expected="boolean",
                actual=_safe_actual(supported),
            ))
        if supported is True and response_policy in {None, "not_applicable", "uncontrolled"}:
            issues.append(ValidationIssue(
                "PY-MN-021", "$.node_model.addressing.broadcast.response_policy",
                "broadcast support requires a collision-safe response policy",
                expected="a defined collision-safe policy", actual=_safe_actual(response_policy),
            ))
        if supported is False and response_policy != "not_applicable":
            issues.append(ValidationIssue(
                "PY-MN-022", "$.node_model.addressing.broadcast.response_policy",
                "disabled broadcast requires not_applicable response policy",
                expected="not_applicable", actual=_safe_actual(response_policy),
            ))
    ...
    if not isinstance(mt_supported, bool):
        issues.append(ValidationIssue(
            "PY-MN-027", "$.node_model.multi_target.supported",
            "multi_target supported must be boolean", expected="boolean",
            actual=_safe_actual(mt_supported),
        ))
    if partial not in PARTIAL_FAILURE_POLICIES:
        issues.append(ValidationIssue(
            "PY-MN-023", "$.node_model.multi_target.partial_failure_policy",
            "unsupported partial-failure policy",
            expected=str(sorted(PARTIAL_FAILURE_POLICIES)), actual=_safe_actual(partial),
        ))
    if mt_supported is True and partial == "not_applicable":
        issues.append(ValidationIssue(
            "PY-MN-024", "$.node_model.multi_target.partial_failure_policy",
    ...
    if mt_supported is False and partial != "not_applicable":
        issues.append(ValidationIssue(
            "PY-MN-025", "$.node_model.multi_target.partial_failure_policy",
            "disabled multi-target support requires not_applicable",
            expected="not_applicable", actual=_safe_actual(partial),
        ))

    allowed_scope_values = {
        "protocol_session": {"global", "connection", "per_node", "group"},
        "secure_session": {"global", "connection", "per_node", "group"},
        "sequence": {"global", "connection", "per_node", "per_node_session", "group"},
        "correlation": {"global", "connection", "per_node", "per_node_session", "group"},
    }
    for field, allowed in allowed_scope_values.items():
        if field not in scope:
            issues.append(ValidationIssue(
                "PY-MN-026", f"$.node_model.scope.{field}", "scope field is required",
                expected="explicit scope", actual="missing",
            ))
        elif scope.get(field) not in allowed:
            issues.append(ValidationIssue(
                "PY-MN-028", f"$.node_model.scope.{field}",
                "scope value is not controlled", expected=str(sorted(allowed)),
                actual=_safe_actual(scope.get(field)),
            ))
    group_scopes = {
        scope.get("protocol_session"), scope.get("secure_session"),
        scope.get("sequence"), scope.get("correlation"),
    }
    if "group" in group_scopes:
        ...
        if not isinstance(security_profile, dict) or security_profile.get("group_session_profile") in {None, "not_applicable"}:
            issues.append(ValidationIssue(
                "PY-MN-030", "$.security_profile.group_session_profile",
                "group-scoped session state requires an explicit group session profile",
                expected="defined profile",
                actual=_safe_actual(_get(security_profile, "group_session_profile")),
            ))

    identity_conflict_policy = lifecycle.get("identity_conflict_policy")
    if identity_conflict_policy not in {"reject", "quarantine", "reject_and_quarantine"}:
        issues.append(ValidationIssue(
            "PY-MN-031", "$.node_model.lifecycle.identity_conflict_policy",
    ...
    if lifecycle.get("address_reuse_requires_new_connection_generation") is not True:
        issues.append(ValidationIssue(
            "PY-MN-032", "$.node_model.lifecycle.address_reuse_requires_new_connection_generation",
            "address reuse must create a new connection generation",
            expected="true", actual=_safe_actual(lifecycle.get("address_reuse_requires_new_connection_generation")),
        ))
    if lifecycle.get("address_reuse_requires_new_secure_session") is not True:
        issues.append(ValidationIssue(
            "PY-MN-033", "$.node_model.lifecycle.address_reuse_requires_new_secure_session",
            "address reuse must create a new secure session",
            expected="true", actual=_safe_actual(lifecycle.get("address_reuse_requires_new_secure_session")),
        ))
    ...
    if not isinstance(per_node, int) or isinstance(per_node, bool) or per_node < 1:
        issues.append(ValidationIssue(
            "PY-MN-034", "$.node_model.resources.maximum_pending_requests_per_node",
            "per-Node pending requests must be bounded", expected=">= 1",
            actual=_safe_actual(per_node),
        ))
    if not isinstance(total, int) or isinstance(total, bool) or total < 1:
        issues.append(ValidationIssue(
            "PY-MN-035", "$.node_model.resources.maximum_total_pending_requests",
            "total pending requests must be bounded", expected=">= 1",
            actual=_safe_actual(total),
        ))
    if (
        isinstance(per_node, int) and not isinstance(per_node, bool)
        and isinstance(total, int) and not isinstance(total, bool) and total < per_node
    ):
        issues.append(ValidationIssue(
            "PY-MN-036", "$.node_model.resources.maximum_total_pending_requests",
            "total bound cannot be below the per-Node bound",
            expected=f">= {per_node}", actual=str(total),
        ))
    ...
    if target_scope not in {"single_node", "multi_target"}:
        issues.append(ValidationIssue(
            "PY-MN-038", "$.node_model.firmware_update.target_scope",
            "unsupported Firmware Update target scope",
            expected="single_node or multi_target", actual=_safe_actual(target_scope),
        ))
    if concurrency not in CONCURRENCY_POLICIES:
        issues.append(ValidationIssue(
            "PY-MN-039", "$.node_model.firmware_update.concurrency",
            "unsupported Firmware Update concurrency",
            expected=str(sorted(CONCURRENCY_POLICIES)), actual=_safe_actual(concurrency),
        ))
    if concurrency == "bounded_parallel":
        ...
        if not isinstance(parallel, int) or isinstance(parallel, bool) or parallel < 1:
            issues.append(ValidationIssue(
                "PY-MN-040", "$.node_model.firmware_update.maximum_parallel_updates",
                "bounded_parallel requires a positive maximum_parallel_updates",
                expected=">= 1", actual=_safe_actual(parallel),
            ))
        elif (
            isinstance(maximum_online, int) and not isinstance(maximum_online, bool)
            and parallel > maximum_online
        ):
            issues.append(ValidationIssue(
                "PY-MN-041", "$.node_model.firmware_update.maximum_parallel_updates",
                "parallel update bound cannot exceed maximum_online_nodes",
                expected=f"<= {maximum_online}", actual=str(parallel),
            ))
    if target_scope == "multi_target" and mt_supported is not True:
        issues.append(ValidationIssue(
            "PY-MN-042", "$.node_model.firmware_update.target_scope",
...
def validate_path(
    ...
    try:
        ...
        return [ValidationIssue(
            "PY-YAML-001", str(path), f"unable to load Protocol YAML or schema: {exc}"
        )]
